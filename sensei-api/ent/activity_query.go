...
func (aq *ActivityQuery) QueryTasks() *TaskQuery {
...
func (aq *ActivityQuery) WithTasks(opts ...func(*TaskQuery)) *ActivityQuery {
	query := (&TaskClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(activity.FieldCreationDate).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Activity.Query().
//		Select(activity.FieldCreationDate).
//		Scan(ctx, &v)
...
func (aq *ActivityQuery) loadTasks(ctx context.Context, query *TaskQuery, nodes []*Activity, init func(*Activity), assign func(*Activity, *Task)) error {
	...
	query.Where(predicate.Task(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(activity.TasksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.activity_tasks
		if fk == nil {
			return fmt.Errorf(`foreign-key "activity_tasks" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
