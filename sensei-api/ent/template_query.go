...
func (tq *TemplateQuery) QueryTemplateTasks() *TemplateTaskQuery {
	query := (&TemplateTaskClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TemplateQuery) Clone() *TemplateQuery {
	...
	return &TemplateQuery{
		...
		withTemplateTasks: tq.withTemplateTasks.Clone(),
...
func (tq *TemplateQuery) WithTemplateTasks(opts ...func(*TemplateTaskQuery)) *TemplateQuery {
	query := (&TemplateTaskClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withTemplateTasks = query
	return tq
}
...
//		GroupBy(template.FieldCreationDate).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Template.Query().
//		Select(template.FieldCreationDate).
//		Scan(ctx, &v)
...
func (tq *TemplateQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Template, error) {
	...
	if query := tq.withUser; query != nil {
		if err := tq.loadUser(ctx, query, nodes, nil,
			func(n *Template, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withTemplateTasks; query != nil {
		if err := tq.loadTemplateTasks(ctx, query, nodes,
			func(n *Template) { n.Edges.TemplateTasks = []*TemplateTask{} },
			func(n *Template, e *TemplateTask) { n.Edges.TemplateTasks = append(n.Edges.TemplateTasks, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TemplateQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Template, init func(*Template), assign func(*Template, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Template)
	for i := range nodes {
		if nodes[i].user_templates == nil {
			continue
		}
		fk := *nodes[i].user_templates
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_templates" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TemplateQuery) loadTemplateTasks(ctx context.Context, query *TemplateTaskQuery, nodes []*Template, init func(*Template), assign func(*Template, *TemplateTask)) error {
	...
	query.Where(predicate.TemplateTask(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(template.TemplateTasksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.template_template_tasks
		if fk == nil {
			return fmt.Errorf(`foreign-key "template_template_tasks" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "template_template_tasks" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
