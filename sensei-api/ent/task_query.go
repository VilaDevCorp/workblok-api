...
func (tq *TaskQuery) QueryActivity() *ActivityQuery {
...
func (tq *TaskQuery) WithActivity(opts ...func(*ActivityQuery)) *TaskQuery {
	query := (&ActivityClient{config: tq.config}).Query()
...
//
//	client.Task.Query().
//		GroupBy(task.FieldCreationDate).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if tq.withActivity != nil || tq.withUser != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := tq.withActivity; query != nil {
		if err := tq.loadActivity(ctx, query, nodes, nil,
			func(n *Task, e *Activity) { n.Edges.Activity = e }); err != nil {
			return nil, err
		}
	}
...
func (tq *TaskQuery) loadActivity(ctx context.Context, query *ActivityQuery, nodes []*Task, init func(*Task), assign func(*Task, *Activity)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Task)
	for i := range nodes {
		if nodes[i].activity_tasks == nil {
			continue
		}
		fk := *nodes[i].activity_tasks
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "activity_tasks" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
