...
func (bq *BlockQuery) QueryUser() *UserQuery {
...
func (bq *BlockQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (bq *BlockQuery) WithUser(opts ...func(*UserQuery)) *BlockQuery {
	query := (&UserClient{config: bq.config}).Query()
...
//
//	client.Block.Query().
//		GroupBy(block.FieldCreationDate).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Block.Query().
//		Select(block.FieldCreationDate).
//		Scan(ctx, &v)
...
func (bq *BlockQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Block, error) {
	...
	if query := bq.withUser; query != nil {
		if err := bq.loadUser(ctx, query, nodes, nil,
			func(n *Block, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (bq *BlockQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Block, init func(*Block), assign func(*Block, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Block)
	for i := range nodes {
		if nodes[i].user_blocks == nil {
			continue
		}
		fk := *nodes[i].user_blocks
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_blocks" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (bq *BlockQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(block.Table, block.Columns, sqlgraph.NewFieldSpec(block.FieldID, field.TypeUUID))
	_spec.From = bq.sql
