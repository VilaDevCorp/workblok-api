...
type User struct {
	...
	// EmailValid holds the value of the "EmailValid" field.
...
func (e UserEdges) BlocksOrErr() ([]*Block, error) {
	if e.loadedTypes[0] {
		return e.Blocks, nil
	}
...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case user.FieldConfig:
			values[i] = new([]byte)
		case user.FieldEmailValid, user.FieldTutorialCompleted:
	...
	for i := range columns {
		...
		case user.FieldEmailValid:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field EmailValid", values[i])
			} else if value.Valid {
				u.EmailValid = value.Bool
			}
		case user.FieldConfig:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field Config", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &u.Config); err != nil {
...
// QueryBlocks queries the "blocks" edge of the User entity.
func (u *User) QueryBlocks() *BlockQuery {
	return NewUserClient(u.config).QueryBlocks(u)
}
...
func (u *User) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("EmailValid=")
	builder.WriteString(fmt.Sprintf("%v", u.EmailValid))
	builder.WriteString(", ")
