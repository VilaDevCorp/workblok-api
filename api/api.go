...
func GetRouter() *gin.Engine {
	...
	pub.POST("/validate/:email/:code", auth.ValidateAccount)
	pub.POST("/validate/:email/resend", auth.ResendValidationCode)
	pub.POST("/forgotten-password/:email", auth.SendForgottenPasswordCode)
	pub.POST("/reset-password/:email/:code", auth.ResetPassword)
	pub.POST("/register", auth.SignUp)
	priv := api.Group("/api/private")
	priv.Use(middleware.Authentication())
	pub.GET("/health", health)
	pub.POST("/refresh-token", auth.RefreshToken)
